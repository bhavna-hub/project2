...
Our main question of interest; Is there a difference in case proportion among countries in the WHO region of Americas that implemented a mask mandate by April 1? And if so did the countries with a mask policy in place by April 1 have lower case proportions? Masks prevent air from flowing from one person to another. This prevents highly infectious diseases like COVID19 from spreading. So it can be hypothesized that the countries requiring masks would have a lower case proportion.
...
```{r}
### CREATE SCATTER PlOT OF CASE PROPORTION VS CASE MORTALITY RATE WITH FILL AS MASK POLICY IN APRIL
