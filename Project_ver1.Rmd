...
Our main question of interest is whether countries that had a mask mandate in place before April 1 had different case proportions by August 31 in the WHO region of the Americas. If there is, did countries with mask mandates have lower case proportions? Because masks prevent body fluid from traveling from one person to another, countries that did have mask policies may have lower case counts. It will help policymakers legislate to bring down COVID cases in the continuing pandemic. It will also help health workers deal diseases similar to COVID in the future.
...
```{r}
g1 = df.augresult %>% ggplot() +
  geom_histogram(mapping =aes(case_proportion)) +
  xlab("Case proportion with masking in April")

g2 =  df.augresult.nm %>% ggplot() +
  geom_histogram(mapping = aes(case_proportion)) +
  xlab("Case proportion with no masking in April")
