...
for (x in 1:nrow(covid)) {
  ...
  if (covid$Cumulative_cases[x]!=0 && covid$Cumulative_cases[x-1]==0 & covid$Cumulative_cases[x+100]>4000) {
...
### OMIT CHINA OUTLIER ###################################################################################
test.data<-dat.growthrates[-c(17),]
```
...
plot(gc_fit, lwd=3, main='United States First 200 Days')

gc_fit2<-SummarizeGrowth(covid.x,dat.200days[,'AZ'])
plot(gc_fit2, lwd=3, main='Azerbaijan First 200 Days')

gc_fit3<-SummarizeGrowth(covid.x,dat.200days[,'BO'])
plot(gc_fit3, lwd=3, main='Bolivia First 200 Days')

gc_fit4<-gc_fit3<-SummarizeGrowth(covid.x,dat.200days[,'PH'])
plot(gc_fit3, lwd=3, main='Philippines First 200 Days')
```
...
From the scatterplot of 200 Day Growth rate below, we can see that China may be an outlier which could skew the results of the model. It may be worth considering omitting China from the model for this reason. Although China was effective at containing the spread of the virus after just 100 days, its initial growth rate was very high, which likely resulted in the high observed growth rate.
...
If we consider China's logistic growth curve, we can see why it possesses such a high r value. The rate of initial spread was very high, but it levelled off quickly after containment measures were implemented.


```{r}
gc_fit6<-gc_fit3<-SummarizeGrowth(covid.x,dat.200days[,'CN'])
plot(gc_fit6, lwd=3, main='China First 200 Days')
...
boxplot(unlist(to.plot.lessmasks),
        ...
        names=c('Less than 100 Days of Masks (n=19)', 'More than 100 Days (n=66)'),
...
boxplot(unlist(to.plot.lesslockdown),
        ...
        names=c('Less than 100 Days of Lockdown (n=65)', 'More than 100 Days (n=20)'),
...
Our primary question of interest is whether certain policies enacted at the national and sub-national level may be associated with different outcomes in the 200 day growth rate of the disease as measured by a fitted logistic growth curve. The policies we have examined include mask wearing obligations, testing implementation, lockdown enforcement, and restaurant closures.  In the Oxford dataset, which tracks daily covid cases as well as daily policy implementations through the use of indicator variables, we obtained the sum of days in which policy measures were in place over the 200 day period. Each policy indicator variable took a value of 1 if the given policy was implemented at the national or sub-national (local) level on that day, and 0 if the policy was not enacted at both the national and sub-national level. The 200 day period is defined here with day 1 being the date on which the first case was recorded in the given country. This was done so as to ensure that measurement of growth rate was consistent for all countries in question. Although we should note that this approach fails to take into account other relevant factors, such as seasonal climate and population density. 


The dataset which is implemented in the model contains 85 observations. In order to increase the reliability of the model, we decided to only take into account countries which recorded at least 4000 cases by the 100th day after initial infection. We found that countries below this threshold tended to report more inaccurate or misleading numbers, or numbers which were inconsistent enough (i.e. large jumps or gaps in reported data) which caused fitted logistic growth curves to be unrepresentative of the actual spread of the disease. After doing this, we were left with 86 observations. We then omitted China from the final dataset, since it was identified earlier as being an outlier which could skew the results of the model. Although cases in China levelled off rapidly over just 100 days, its high initial rate of spread followed by its extremely high rate of diminishment caused it's $r$ parameter to be much higher than any other observed $r$ parameters in the dataset of 86 countries. 
...
$$Y_{i}=\beta_0+\beta_1x_{1,i}+\beta_2x_{2,i}+\beta_3x_{3,i}+\beta_4x_{4,i} \quad \quad i=1,...,85$$
...
model_growth.rate.logistic<-lm(data=test.data,formula=unlist(gr)~days_testing+days_mask+days_restaurant_close+days_lockdown)
model_growth.rate.logistic<-lm(data=test.data,formula=unlist(gr)~days_mask)
summary(model_growth.rate.logistic)
plot(model_growth.rate.logistic)
summary(model_growth.rate.logistic)
...
```{r}
###OTHER TESTS
(confint(model_growth.rate.logistic, 'days_mask', level=0.95))
(confint(model_growth.rate.logistic, 'days_restaurant_close', level=0.95))
```
...
```{r}
hist(model_growth.rate.logistic$residuals)
```
